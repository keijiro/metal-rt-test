...
public static class ShaderGraphComputeGen
{
    ...
    public static string Generate(string graphPath, string outputPath)
    {
        ...
        // Shader Graph keywords: any shader keyword referenced by the
        // extracted graph code becomes a multi_compile variant of the
        // kernel (bound per material at dispatch time).
        var keywordPragmas = "";
        var shaderAsset = AssetDatabase.LoadAssetAtPath<Shader>(graphPath);
        if (shaderAsset != null)
        {
            var keywordNames = new HashSet<string>
              (shaderAsset.keywordSpace.keywordNames);
            var graphCode = functions + function + inputsStruct + surfStruct;
            var referenced = new HashSet<string>();
            foreach (Match m in Regex.Matches
                       (graphCode,
                        @"(?:#ifdef\s+|defined\s*\(\s*)([A-Za-z_]\w*)"))
                if (keywordNames.Contains(m.Groups[1].Value))
                    referenced.Add(m.Groups[1].Value);
            keywordPragmas = string.Join("\n", referenced.Select
              (k => $"#pragma multi_compile _ {k}"));
        }
